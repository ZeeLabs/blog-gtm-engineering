...
def extract_metadata_from_post(post_path):
    """Extract metadata from an existing HTML post file."""

    try:
        with open(post_path, "r", encoding="utf-8") as f:
            content = f.read()
    except Exception as e:
        print(f"❌ Could not read post file: {e}")
        return None
    ...
    if desc_match:
        ...
        metadata["description"] = metadata["title"]  # Fallback to title
    ...
    if author_match:
        ...
        metadata["author"] = "Jorge Macias"  # Default

    # Extract Open Graph image
    og_image_match = re.search(
        r'<meta\s+property=["\']og:image["\']\s+content=["\']([^"\']+)["\']', content, re.IGNORECASE
    )
    if og_image_match:
        ...
        metadata["image"] = "../assets/gtm-revenue-system-illustration.webp"  # Default
    ...
    if keywords_match:
        ...
        metadata["tags"] = [tag.title() for tag in tags if tag.strip()]
    else:
